...
class StagedMovies(object):
    ...
    def add_all(self, items):
        ...
        progress_dialog.create(self.STR_ADDON_NAME, STR_ADDING_ALL_MOVIES)
        for item in items:
    ...
    def add_all_with_metadata(self, items):
        ...
        progress_dialog = xbmcgui.DialogProgress()
        progress_dialog.create(self.STR_ADDON_NAME, STR_ADDING_ALL_MOVIES_WITH_METADATA)
        for item in items:
            ...
            if os.path.exists(nfo_path):
                progress_dialog.update(0, line2=item.get_title())
                item.add_to_library()
            progress_dialog.update(0, line2=' ')
        progress_dialog.close()
    ...
    def clean_up_metadata(self):
        ...
        STR_MOVIE_METADATA_CLEANED = self.addon.getLocalizedString(32136)
    ...
    def generate_all_metadata(self, items):
        ...
        progress_dialog = xbmcgui.DialogProgress()
        progress_dialog.create(self.STR_ADDON_NAME, STR_GENERATING_ALL_MOVIE_METADATA)
        for item in items:
            progress_dialog.update(0, line2=item.get_title())
    ...
    def options(self, item):
        ...
        ret = xbmcgui.Dialog().select(
            '{0} - {1} - {2}'.format(
                self.STR_ADDON_NAME, STR_STAGED_MOVIE_OPTIONS, item.get_title()
            ), lines
        )
        if ret >= 0:
    ...
    def view_all(self):
        ...
        STR_CLEAN_UP_METADATA = self.addon.getLocalizedString(32135)
        ...
        ret = xbmcgui.Dialog().select(
            '{0} - {1}'.format(self.STR_ADDON_NAME, STR_STAGED_MOVIES), lines
        )
        if ret >= 0:
...
class StagedTV(object):
    ...
    def add_all_episodes(self, items):
        ...
        progress_dialog = xbmcgui.DialogProgress()
        progress_dialog.create(self.STR_ADDON_NAME, STR_ADDING_ALL_x_EPISODES % show_title)
        for item in items:
            progress_dialog.update(0, line2=item.get_title())
            item.add_to_library()
        progress_dialog.close()
    ...
    def add_all_episodes_with_metadata(self, items):
        ...
        progress_dialog.create(
            self.STR_ADDON_NAME, STR_ADDING_ALL_x_EPISODES_WITH_METADATA % show_title
        )
        for item in items:
            safe_title = clean_name(item.get_title())
            nfo_path = os.path.join(metadata_dir, safe_title + '.nfo')
    ...
    def add_all_shows(self):
        ...
        progress_dialog = xbmcgui.DialogProgress()
        progress_dialog.create(self.STR_ADDON_NAME, STR_ADDING_ALL_TV_SHOWS)
    ...
    def add_all_with_metadata(self):
        ...
        progress_dialog = xbmcgui.DialogProgress()
        progress_dialog.create(self.STR_ADDON_NAME, STR_ADDING_ALL_TV_SHOW_ITEMS_WITH_METADATA)
        staged_tv_items = self.dbh.get_content_items(
            status='staged', mediatype='tvshow', order='Show_Title'
        )
        for item in staged_tv_items:
            ...
            if os.path.exists(nfo_path):
                ...
                item.add_to_library()
            progress_dialog.update(0, line2=' ', line3=' ')
        progress_dialog.close()
    ...
    def episode_options(self, item):
        ...
        ret = xbmcgui.Dialog().select(
            '{0} - {1} - {2}'.format(
                self.STR_ADDON_NAME, STR_STAGED_EPISODE_OPTIONS, item.get_title()
            ), lines
        )
        if ret >= 0:
            if lines[ret] == STR_ADD:
                item.add_to_library()
                self.view_episodes(item.get_show_title())
            elif lines[ret] == STR_REMOVE:
                item.delete()
                self.view_episodes(item.get_show_title())
    ...
    def generate_all_episodes_metadata(self, items):
        ...
        show_title = items[0].get_show_title()
        progress_dialog = xbmcgui.DialogProgress()
        progress_dialog.create(self.STR_ADDON_NAME, STR_GENERATING_ALL_x_METADATA % show_title)
        for item in items:
            progress_dialog.update(0, line2=item.get_title())
            item.create_metadata_item()
        progress_dialog.close()
    ...
    def generate_all_metadata(self):
        ...
        progress_dialog = xbmcgui.DialogProgress()
        progress_dialog.create(self.STR_ADDON_NAME, STR_GENERATING_ALL_TV_SHOW_METADATA)
        staged_tv_items = self.dbh.get_content_items(
            status='staged', mediatype='tvshow', order='Show_Title'
        )
        for item in staged_tv_items:
    ...
    def remove_all_episodes(self, show_title):
        ...
        progress_dialog = xbmcgui.DialogProgress()
        progress_dialog.create(self.STR_ADDON_NAME, STR_REMOVING_ALL_x_EPISODES)
    ...
    def rename_episodes_using_metadata(self, items):
        ...
        progress_dialog = xbmcgui.DialogProgress()
        progress_dialog.create(
            self.STR_ADDON_NAME, STR_RENAMING_x_EPISODES_USING_METADATA % show_title
        )
        for item in items:
            progress_dialog.update(0, line2=item.get_title())
            item.rename_using_metadata()
    ...
    def view_shows(self):
        ...
        ret = xbmcgui.Dialog().select(
            '{0} - {1}'.format(self.STR_ADDON_NAME, STR_STAGED_TV_SHOWS), lines
        )
        if ret >= 0:
