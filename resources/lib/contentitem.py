...
class ContentItem(object):
    '''
    Parent class for MovieItem and EpisodeItem.
    Defines required and convenience methods.
    '''
    ...
    def add_to_library(self):
        ''' Define required method for child classes --
        adds its content to the library '''
    ...
    def create_metadata_item(self):
        ''' Define required method for child classes --
        adds relevent files to metadata folder '''
    ...
    def delete(self):
        ...
        db.DatabaseHandler().remove_content_item(self.path)

    def set_as_staged(self):
        ''' Set the item status as staged in database '''
        db.DatabaseHandler().update_content(self.path, status='staged')

    def get_title(self):
        ''' Return title of video as string '''
        return self.title

    def get_path(self):
        ''' Return path to video in plugin as string '''
        return self.path

    def get_mediatype(self):
        ''' Return mediatype (movie or tvshow) as string '''
...
class MovieItem(ContentItem):
    ...
    def add_to_library(self):
        ...
        safe_title = utils.clean_name(self.title)
        ...
        db.DatabaseHandler().update_content(self.path, status='managed')
...
class EpisodeItem(ContentItem):
    ...
    def add_to_library(self):
        ...
        safe_showtitle = utils.clean_name(self.show_title)
    ...
    def create_metadata_item(self):
        ...
        if not os.path.exists(strm_path):
            ...
            old_renamed = glob(
                os.path.join(show_dir, '*[0-9]x[0-9]* - {0}.nfo'.format(safe_title_no_0x0))
            )
            if old_renamed:
            ...
            if new_title != self.title:
                ...
                db.DatabaseHandler().update_content(self.path, title=self.title)
    ...
    def rename(self, name):
        ...
        if os.path.isdir(metadata_dir):
            ...
            new_title_path = os.path.join(metadata_dir, utils.clean_name(name))
